  ...
  return (
    <div className="app">
      ...
      <main className="container">
                ...
                .map(([id, dl]) => (
                  <div key={id} className={`download-card ${dl.status}`}>
                    <div className="download-status">
                      {dl.status === 'downloading' && <span className="spinner"></span>}
                      {dl.status === 'completed' && <span className="icon">✓</span>}
                      {dl.status === 'error' && <span className="icon">✕</span>}
                      {dl.status === 'queued' && <span className="icon">⏳</span>}
                    </div>
                    ...
                    <div className="download-content">
                      ...
                      <p className="time-text">{formatTime(dl.created_at)}</p>
                    </div>
        ...
        {files.length > 0 && (
          <section className="section">
            <h2>Downloaded Files ({files.length})</h2>
            <div className="files-table">
              ...
              {files.map((file, idx) => (
                <div key={idx} className="table-row">
                  ...
                  <div className="col-date">{formatTime(file.modified)}</div>
                </div>
              ))}
            </div>
          </section>
